...
가장 먼저 `POST /api/v1/auth/token`의 통합 테스트를 작성한다.
...
토큰 발급 후 실제 보호 API 접근까지 이어지는 테스트를 작성한다.
...
### 3.2 local/test client 생성 방식 정리
...
### 3.3 API Client 등록 API

토큰 인증 흐름 검증이 끝난 뒤 API Client 등록 API를 구현한다.
...
등록 API 이후 API Key 재발급 API를 구현한다.
...
1. 토큰 발급 API 통합 테스트
2. Bearer 토큰 기반 보호 API 통합 테스트
3. README 인증 섹션 정리
4. local/test client 생성 방식 문서화
5. API Client 등록 API 구현
6. API Key 재발급 API 구현
...
현재 바로 착수할 작업은 1번 `토큰 발급 API 통합 테스트`다.
